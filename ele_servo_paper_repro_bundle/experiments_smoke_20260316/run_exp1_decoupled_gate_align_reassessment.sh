...
OUT_ROOT="${OUT_ROOT:-${PROJECT_ROOT}/experiments_smoke_20260316/models/exp1_gate_align_reassessment_v1}"
...
STAGE1_EPOCHS="${STAGE1_EPOCHS:-24}"
...
if [[ "${MODALITY_DROP_PROB}" != "0" && "${MODALITY_DROP_PROB}" != "0.0" ]]; then
  QUALITY_ARGS="${QUALITY_ARGS} --quality-drop-prob ${MODALITY_DROP_PROB}"
fi

echo "[gate-align-reassessment] OUT_ROOT=${OUT_ROOT}"
echo "[gate-align-reassessment] MODALITY_DROP_PROB=${MODALITY_DROP_PROB}"
echo "[gate-align-reassessment] FEATURE_MODE=${FEATURE_MODE}"
...
echo "[gate-align-reassessment] done"
echo "[gate-align-reassessment] inspect:"
echo "  ${STAGE2_DIR}/report.json"
echo "  ${STAGE2_DIR}/test_quality_gate_summary.json"
