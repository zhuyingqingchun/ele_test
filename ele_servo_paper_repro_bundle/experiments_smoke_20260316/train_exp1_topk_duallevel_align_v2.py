...
from experiments_smoke_20260316.exp1_topk_duallevel_align_v2_models import Stage3TopKDualLevelAlignV2, Stage4TopKDualLevelAlignV2, info_nce
...
def build_multiview_text_cache(output_dir: Path, records: list, text_backbone: str, qwen_path: Path, device: torch.device, text_batch_size: int):
    ...
    cache_path = output_dir / f"text_multiview_embeddings__{text_backbone}.pt"
...
def compute_total_loss(outputs, labels: torch.Tensor, recs: list, class_names: list[str], prototype_bank: SoftPrototypeBank | None, lambda_cfg: dict[str, float]):
    ...
    if prototype_bank is not None:
        proto_loss = prototype_bank.prototype_alignment_loss(labels, outputs.signal_embedding, outputs.evidence_text_embedding, outputs.mechanism_text_embedding)
    quality_loss = outputs.quality_loss if outputs.quality_loss is not None else outputs.logits.new_zeros(())
    total = cls_loss + lambda_cfg["combined"] * align_combined + lambda_cfg["evidence"] * align_evidence + lambda_cfg["mechanism"] * align_mechanism + lambda_cfg["contrast"] * contrast_loss + lambda_cfg["listwise"] * listwise + lambda_cfg["hardneg"] * hard_neg + lambda_cfg["proto"] * proto_loss + lambda_cfg["quality"] * quality_loss
    parts = {"cls": float(cls_loss.item()), "combined": float(align_combined.item()), "evidence": float(align_evidence.item()), "mechanism": float(align_mechanism.item()), "contrast": float(contrast_loss.item()), "listwise": float(listwise.item()), "hardneg": float(hard_neg.item()), "proto": float(proto_loss.item()), "quality": float(quality_loss.item())}
    return total, parts
...
def main() -> None:
    ...
    parser.add_argument("--lambda-combined", type=float, default=0.25)
    ...
    parser.add_argument("--lambda-listwise", type=float, default=0.25)
    parser.add_argument("--lambda-hardneg", type=float, default=0.20)
    parser.add_argument("--lambda-proto", type=float, default=0.15)
    ...
    if int(args.stage) == 3:
        model = Stage3TopKDualLevelAlignV2(input_dims, len(class_names), int(text_dim), model_dim=int(args.model_dim), token_dim=int(args.token_dim), quality_hidden_dim=int(args.quality_hidden_dim), modality_drop_prob=float(args.quality_drop_prob), quality_min_gate=float(args.quality_min_gate), fault_gate_hidden_dim=int(args.fault_gate_hidden_dim)).to(device)
    else:
        model = Stage4TopKDualLevelAlignV2(input_dims, len(class_names), int(text_dim), model_dim=int(args.model_dim), token_dim=int(args.token_dim), num_layers=int(args.llm_layers), nhead=int(args.llm_heads), dim_feedforward=int(args.llm_ff), pool=str(args.pool), quality_hidden_dim=int(args.quality_hidden_dim), modality_drop_prob=float(args.quality_drop_prob), quality_min_gate=float(args.quality_min_gate), fault_gate_hidden_dim=int(args.fault_gate_hidden_dim)).to(device)
    ...
    best_val = -math.inf
    history = []
    lambda_cfg = {"combined": float(args.lambda_combined), "evidence": float(args.lambda_evidence), "mechanism": float(args.lambda_mechanism), "contrast": float(args.lambda_contrast), "listwise": float(args.lambda_listwise), "hardneg": float(args.lambda_hardneg), "proto": float(args.lambda_proto), "quality": float(args.lambda_quality)}
    ...
    for epoch in range(1, int(args.epochs) + 1):
        ...
        correct = 0
        loss_sum = 0.0
        part_sums = {k: 0.0 for k in ["cls", "combined", "evidence", "mechanism", "contrast", "listwise", "hardneg", "proto", "quality"]}
        ...
        row = {"epoch": epoch, "loss": loss_sum / max(1, total), "scenario_accuracy": correct / max(1, total), "val_scenario_accuracy": float(val_metrics["scenario_accuracy"])}
    ...
    scores_csv = args.output_dir / "test_topk_consistency_scores.csv"
    summary_json = args.output_dir / "test_topk_consistency_summary.json"
    write_rows_csv(scores_csv, rows)
    save_summary_json(summary_json, summarize_topk_rows(rows))
    ...
    report = {
        ...
        "topk_scores_csv": str(scores_csv),
        "topk_summary_json": str(summary_json),
        "history": history,
