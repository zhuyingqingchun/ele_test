...
class Stage4FaultAwareMultiViewClassifier(_CommonMultiView):
    ...
    def _apply_modality_gates(self, signal_tokens: Tensor, token_slices: dict[str, tuple[int, int]], modality_gates: Tensor) -> Tensor:
        # Avoid inplace operations by building a list and concatenating
        parts = []
        prev_end = 0
        for idx, name in enumerate(self.branch_names):
            start, end = token_slices[name]
            # Add tokens before this modality (if any)
            if start > prev_end:
                parts.append(signal_tokens[:, prev_end:start, :])
            # Add gated tokens for this modality
            gated = signal_tokens[:, start:end, :] * modality_gates[:, idx].view(-1, 1, 1)
            parts.append(gated)
            prev_end = end
        # Add remaining tokens (if any)
        if prev_end < signal_tokens.shape[1]:
            parts.append(signal_tokens[:, prev_end:, :])
