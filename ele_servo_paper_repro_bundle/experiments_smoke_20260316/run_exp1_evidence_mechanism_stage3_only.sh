...
mkdir -p "${OUT_ROOT}"
STAGE2_DIR="${PROJECT_ROOT}/experiments_smoke_20260316/models/exp1_fault_aware_gate_multiview_align_v1/stage2_signal_fusion"
STAGE3_DIR="${OUT_ROOT}/stage3_fault_aware_multiview"
STAGE4_DIR="${OUT_ROOT}/stage4_fault_aware_multiview_llm"

# 检查 Round 20 Stage 2 模型是否存在
if [[ ! -f "${STAGE2_DIR}/best.pt" ]]; then
    echo "Error: Round 20 Stage 2 model not found at ${STAGE2_DIR}/best.pt"
    echo "Please run Round 20 experiment first or check the path."
    exit 1
fi

echo "Using Round 20 Stage 2 model: ${STAGE2_DIR}/best.pt"
echo "Test accuracy: 94.51%"
echo ""
...
echo "[Round 23 - Stage 3 Only] Starting Stage 3 (Fault-aware Multi-view)..."
...
if [[ "${RUN_STAGE4}" == "1" ]]; then
  echo "[Round 23 - Stage 3 Only] Starting Stage 4 (Fault-aware Multi-view LLM)..."
  "${PYTHON_PATH}" experiments_smoke_20260316/train_exp1_fault_aware_gate_multiview.py \
...
echo "[Round 23 - Stage 3 Only] All stages completed!"
echo "Results:"
